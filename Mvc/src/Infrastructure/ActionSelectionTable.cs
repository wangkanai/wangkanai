...
internal sealed class ActionSelectionTable<TItem>
{
   ...
   public static ActionSelectionTable<Endpoint> Create(IEnumerable<Endpoint> endpoints)
      => CreateCore(0,
                    endpoints.Where(e => e.GetType() == typeof(Endpoint)),
                    e => e.Metadata.GetMetadata<ActionDescriptor>()?.RouteValues?.Keys,
                    (e, key) =>
                    {
                       string? value = null;
      ...
      Func<T, string, string>       getRouteValue)
   {
      ...
      return new(version, routeKeys.ToArray(), ordinalEntries, ordinalIgnoreCaseEntries);
   }
