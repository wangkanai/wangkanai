...
public sealed class RandomAccessQueue<T> : ICollection<T>, ICollection, ICloneable
{
   ...
   public RandomAccessQueue(int capacity)
      => _buffer = new T[Math.Max(capacity, DefaultCapacity)];
   ...
   /// <summary>Creates a new queue with the same contents as this queue. The queues are separate, however - adding an item to the returned queue doesn't affect this queue or vice versa.</summary>
   /// <returns></returns>
   ...
   /// <summary>An object reference to synchronize on when using the queue from multiple threads. This reference is not used for anywhere in the class itself. The same reference will always be returned for the same queue, and this will never be the same as the reference returned for a different queue, even a clone.</summary>
   public object SyncRoot { get; } = new();

   /// <summary>Copies the elements of the queue to the given array, beginning at the given index.</summary>
   ...
   {
      var newCapacity = Math.Max(Count, DefaultCapacity);
   ...
   /// <summary>Searches the queue for a given item and returns the index of the item if found. The search is performed using a binary search algorithm, and the comparison logic is determined by the specified comparer.</summary>
   ...
   /// <returns>The zero-based index of the specified item in the queue, if found; otherwise, a negative number that is the bitwise complement of the index of the next element that is larger than the value, or, if there is no larger element, the bitwise complement of
   /// <see cref="Count"/>.</returns>
   ...
   /// <returns>The zero-based index of the specified value if it is found, or a negative number which is the bitwise complement of the index of the next element that is larger than the specified value (or if there is no larger element, the bitwise complement of
   /// <see cref="Count"/>).</returns>
   ...
   /// <returns>The zero-based index of the specified value in the queue if found; otherwise, a negative number that is the bitwise complement of the index of the next element that is larger than
   /// <paramref name="value"/> or, if no such element exists, the bitwise complement of <see cref="Count"/>. </returns>
