...
{
   /// <summary>Reverse the original comparer, if it was already a reverse comparer, it will return the original comparer.</summary>
   ...
   /// <summary>Combine a comparer with another comparer to implement a compound comparer.</summary>
   public static IComparer<T> ThenBy<T>(this IComparer<T> first, IComparer<T> second)
      => new LinkedComparer<T>(first, second);

   /// <summary>Combine a comparer with a projection to implement a compound comparer.</summary>
