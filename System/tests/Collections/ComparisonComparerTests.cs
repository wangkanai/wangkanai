...
{
	[Fact] public void ConstructorWithNull() => Assert.Throws<ArgumentNullException>(() => new ComparisonComparer<string>(null));
	[Fact] public void CreateComparisonWithNull() => Assert.Throws<ArgumentNullException>(() => ComparisonComparer<string>.CreateComparison(null));

	[Fact]
	public void CreateAndCall()
	{
		var mock = new Mock<Comparison<string>>();
		mock.Setup(x => x("hello", "there")).Returns(5);
		mock.Setup(x => x("x", "y")).Returns(-3);
		mock.Setup(x => x("throw", "exception")).Throws<Exception>();
	...
	public void CreateAndCallComparison()
	{
		...
		mock.Setup(x => x.Compare("hello", "there")).Returns(5);
		mock.Setup(x => x.Compare("x", "y")).Returns(-3);
		mock.Setup(x => x.Compare("throw", "exception")).Throws<Exception>();
