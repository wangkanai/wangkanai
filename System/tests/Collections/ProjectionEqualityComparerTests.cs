...
public class ProjectionEqualityComparerTests
{
   ...
   [Fact]
   public void ProjectionToStringWithExplicitType() => ProjectionEqualityComparer.Create((NameAndNumber x) => x.Name).AssertNameComparison();

   [Fact]
   public void ProjectionToNumberWithExplicitType() => ProjectionEqualityComparer.Create((NameAndNumber x) => x.Number).AssertNumberComparison();

   [Fact]
   public void ProjectionToStringWithIgnored() => ProjectionEqualityComparer.Create(x => x.Name, A10).AssertNameComparison();

   [Fact]
   public void ProjectionToNumberWithIgnored() => ProjectionEqualityComparer.Create(x => x.Number, A10).AssertNumberComparison();

   [Fact]
   public void ProjectionToStringWithGenericType() => ProjectionEqualityComparer<NameAndNumber>.Create(x => x.Name).AssertNameComparison();

   [Fact]
   public void ProjectionToNumberWithGenericType() => ProjectionEqualityComparer<NameAndNumber>.Create(x => x.Number).AssertNumberComparison();

   [Fact]
   public void NullProjection() => Assert.Throws<ArgumentNullException>(() => new ProjectionEqualityComparer<NameAndNumber, string>(null));
...
/// <summary>Utility method extension to help perform appropriate assertions with the given comparer.</summary>
