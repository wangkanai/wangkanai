...
{
   private readonly DictionaryByType subject;

   public DictionaryByTypeTests() => subject = new();
   ...
   {
      subject.Add("Hi");
      Assert.Throws<ArgumentException>(() => subject.Add("There")
                                      );
   }
   ...
   public void GetFailsForMissingType() =>
      Assert.Throws<KeyNotFoundException>(() => subject.Get<string>()
                                         );
