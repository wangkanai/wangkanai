...
public sealed class DeviceService : IDeviceService
{
   ...
   public DeviceService(IUserAgentService userAgentService) => _userAgentService = userAgentService;
   ...
   private static bool IsTablet(string agent) => agent.SearchContains(TabletCollection.KeywordsSearchTrie);

   private static bool IsMobile(string agent) =>
      agent.Length >= 4 && agent.SearchStartsWith(MobileCollection.PrefixesSearchTrie) ||
      agent.SearchContains(MobileCollection.KeywordsSearchTrie);

   private static bool IsTV(string agent) => agent.ContainsLower(Device.Tv) || agent.Contains("bravia", StringComparison.Ordinal);
}