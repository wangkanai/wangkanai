...
public class KmpPrefixTrie : IPrefixTrie
{
   ...
   /// <summary>Creates optimized prefix Trie using static keyword list, combines prefix search Trie and Knuth–Morris–Pratt algorithm optimizations for each separate Trie path separately by creating a tables for each possible search path
   /// <see href="https://en.wikipedia.org/wiki/Knuth%E2%80%93Morris%E2%80%93Pratt_algorithm"/> <see href="https://en.wikipedia.org/wiki/Trie"/>
   /// </summary>
   ...
   private KmpPrefixTrie(string[]? keywords, int pos)
   {
                                           ...
                                           .Select(x => (x.Key, x)))
      {
         ...
         {
            _lookup[key - lowerBound] = new(null, pos + 1);
         }
         else
         {
            _lookup[key - lowerBound] = new(newKeys, pos + 1);
         }
      }
