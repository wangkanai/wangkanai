...
public class AuditStoreTests
{
   ...
   public async ValueTask CreateAsync_SavesToDbContext()
   {
      ...
      Assert.Equal(1,        await context.Audits.CountAsync(TestContext.Current.CancellationToken));
      Assert.Equal(audit.Id, (await context.Audits.FirstOrDefaultAsync(TestContext.Current.CancellationToken))?.Id);
   }
   ...
   public async ValueTask CreateAsync_WithoutAutoSave_DoesNotSaveChanges()
   {
      ...
      // Assert
      Assert.True(result.IsSuccess);
      Assert.Equal(0, await context.Audits.CountAsync(TestContext.Current.CancellationToken));
   }
