...
public static class DecoratorServiceExtensions
{
   ...
   public static void AddDecorator<TService>(this IServiceCollection services)
   {
      ...
      if (registration.ImplementationInstance != null)
      {
         ...
         services.Add(new(typeof(Decorator<TService>), inner, ServiceLifetime.Transient));
         services.Add(new(type, registration.ImplementationInstance));
      }
      else if (registration.ImplementationFactory != null)
      {
         services.Add(new(typeof(Decorator<TService>), p => new DisposableDecorator<TService>((TService)registration.ImplementationFactory(p)), registration.Lifetime));
      }
      else
      {
