...
public class MarkdownViewEngine : IMarkdownViewEngine
{
   ...
   public MarkdownPageResult FindPage(ActionContext context, string pageName)
   {
      ...
      {
         return new(pageName, Enumerable.Empty<string>());
      }

      var cacheResult = LocatePageFromViewLocations(context, pageName, false);
      if (cacheResult.Success)
      {
         var razorPage = cacheResult.ViewEntry.PageFactory();
         return new(pageName, razorPage);
      }

      return new(pageName, cacheResult.SearchedLocations!);
   }
   ...
   public MarkdownPageResult GetPage(string executingFilePath, string pagePath) => throw new NotImplementedException();
   ...
   private MarkdownViewLocationCacheResult LocatePageFromPath(string? executingFilePath, string pagePath, bool isMainPage)
   {
      ...
      if (!ViewLookupCache.TryGetValue(cacheKey, out MarkdownViewLocationCacheResult? cacheResult))
      {
         ...
         if (cacheResult == null)
         {
            cacheResult = new(new[] { applicationRelativePath });
         }
      ...
      bool                  isMainPage)
   {
      ...
      if (factoryResult.Success)
      {
         ...
         return new(
                    new(factoryResult.MarkdownPageFactory, relativePath),
                    viewStartPages);
      }
      ...
      bool          isMainPage)
   {
      ...
      if (expandersCount > 0)
      {
         expanderValues         = new(StringComparer.Ordinal);
      ...
      MarkdownViewLocationCacheKey        cacheKey)
   {
      ...
      if (cacheResult == null)
      {
         cacheResult = new(searchedLocations);
      }
      ...
      HashSet<IChangeToken> expirationTokens)
   {
      ...
      foreach (var filePath in MarkdownFileHierarchy.GetViewStartPaths(path))
      {
         ...
         if (result.Success)
         {
            viewStartPages.Insert(0, new(result.MarkdownPageFactory, filePath));
         }
      }
