...
internal static class CompiledMarkdownActionDescriptorBuilder
{
      ...
      FilterCollection         globalFilters)
   {
      ...
      var filters = globalFilters.Select(f => new FilterDescriptor(f, FilterScope.Global)).Concat(applicationModel.Filters.Select(f => new FilterDescriptor(f, FilterScope.Action)))
                                 .ToArray();
